...
useEffect(() => {
  const fetchProjects = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:8000/${userId}/projects`);
...
async function fetchRequiredDetails() {
  try {
    // Tasks
    const taskResponse = await axios.get(`http://localhost:8000/tasks/${user._id}`);
    ...
    const projectsResponse = await axios.get(
      `http://localhost:8000/${user._id}/projects`
    );
...
const calculateStatus = (project) => {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8000/${user._id}/members`);
